...
export default function Hero() {
  return (
    <section className="relative w-full pt-32 pb-16 md:pt-48 md:pb-32 flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      ...
      {/* Background Map - Clamped to 100vw to prevent overflow */}
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-200/50 dark:bg-white/5 border border-slate-300 dark:border-white/10 backdrop-blur-md mb-6">
          ...
          <span className="text-xs uppercase tracking-widest font-bold opacity-80">No.1 Travel App</span>
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold mb-6 leading-[1.1] tracking-tight">
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-base sm:text-lg md:text-xl opacity-70 max-w-2xl mx-auto mb-10 leading-relaxed px-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-col sm:flex-row w-full sm:w-auto gap-4 px-4">
          ...
          <button className="w-full sm:w-auto px-8 py-4 bg-slate-200 dark:bg-white/5 border border-transparent dark:border-white/10 rounded-full font-bold text-lg hover:bg-slate-300 dark:hover:bg-white/10 backdrop-blur-sm transition-all flex items-center justify-center gap-2"><PlayCircle size={20} /> Watch Demo</button>
        </motion.div>
      </div>
    </section>
  );
